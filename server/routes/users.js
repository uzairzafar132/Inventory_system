...
const uploadDirectory = path.join(__dirname, 'uploads'); // Update the path as needed
...
router.post("/upload-image", upload.single("image"), async (req, res) => {
	try {
	  ...
	  // Access the decoded token from req.decodedToken
	  
	  const userId ='652506824c0913978240b19f'
	  // Update the user's profileImage field with the image URL
	  const user = await User.findByIdAndUpdate(userId, { profileImage: `/uploads/${uniqueFilename}` }, { new: true });
